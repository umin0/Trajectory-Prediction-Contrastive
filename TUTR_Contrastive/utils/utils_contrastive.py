...
def trajectory_motion_modes(all_trajs, obs_len, n_units=120, smooth_size=3, random_rotation=False):
    """
    Extract representative motion patterns (modes) using k-means.
    all_trajs: [B, T, 2]
    """

    # 짧은 궤적 필터링
    valid_mask = np.array([traj.shape[0] > obs_len for traj in all_trajs])
    if not np.any(valid_mask):
        raise ValueError(f"[ERROR] No valid trajectories longer than obs_len={obs_len}.")
    all_trajs = all_trajs[valid_mask]
    ...
    for traj in all_trajs:
        if traj.shape[0] <= obs_len:
            continue
        motion = traj[obs_len:] - traj[obs_len-1]  # future displacement
        clustering_input.append(motion)

    if len(clustering_input) == 0:
        raise ValueError("[ERROR] All trajectories are too short for motion mode extraction.")

    clustering_input = np.stack(clustering_input, axis=0)  # [N, pred_len, 2]

    if smooth_size is not None and clustering_input.shape[1] > smooth_size:
...
def get_motion_modes(dataset, obs_len, pred_len, n_clusters, dataset_path, dataset_name, smooth_size, random_rotation, traj_seg=False):
    ...
    trajs_list = [traj for traj in trajs_list if traj.shape[0] > obs_len]
    if len(trajs_list) == 0:
        raise ValueError(f"[ERROR] No trajectory has more than obs_len={obs_len} frames.")
...
def difficulty_contrastive_loss(
    z,
    difficulty,
    tau=0.5,
    pos_delta=0.4,
    ...
    """
    Difficulty-aware contrastive loss with explicit negatives
    (Makansi-inspired, masked InfoNCE variant)
    """
    ...
    if not torch.is_tensor(difficulty):
        difficulty = torch.as_tensor(difficulty, device=device, dtype=z.dtype)
    else:
        difficulty = difficulty.to(device=device, dtype=z.dtype)
    difficulty = difficulty.view(-1)  # [B]
    ...
    d_i = difficulty.view(-1, 1)
    d_j = difficulty.view(1, -1)
    d_diff = (d_i - d_j).abs()

    # ----- positive / negative masks -----
    pos_mask = (d_diff < pos_delta)
    neg_mask = (d_diff > neg_delta)
    ...
    loss = -torch.log((numerator[valid] + 1e-20) /
                      (denominator[valid] + 1e-20))
