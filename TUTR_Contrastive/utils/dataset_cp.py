...
class TrajectoryDataset(data.Dataset):
    ...
    def coll_fn(self, scenario_list):
        ...
        for item in scenario_list:
            if len(item) == 4:
                ped_obs_traj, ped_pred_traj, neis_traj, cp_val = item
            else:
                ped_obs_traj, ped_pred_traj, neis_traj = item
                cp_val = 0.0
            cp_vals.append(cp_val)  # ✅ cp 값 저장
        ...
        cp_vals = torch.tensor(cp_vals, dtype=torch.float32).unsqueeze(1)  # ✅ cp 텐서화

        return ped, neis, neis_mask, cp_vals  # ✅ cp 추가 반환
    ...
    def __getitem__(self, idx):
        data = self.scenario_list[idx]  # ✅ 올바른 리스트 이름

        # pkl 구조는 (obs, fut, neighbor) 또는 (obs, fut, neighbor, cp_max)
        if len(data) == 4:
            ped_obs_traj, ped_pred_traj, neis_traj, cp_val = data
        else:
            ped_obs_traj, ped_pred_traj, neis_traj = data
            cp_val = 0.0  # cp 없는 경우 0으로 대체

        return ped_obs_traj, ped_pred_traj, neis_traj, cp_val
