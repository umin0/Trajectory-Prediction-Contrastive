...
from utils.dataset_cp import TrajectoryDataset
...
python cp3_train.py --dataset_path dataset/contrastive_bench_cp/ --dataset_name lis --hp_config config/ig_bench_contrastive.py --gpu 0 --checkpoint checkpoint/lis/contrastive_bench_cp/ --search --search_epochs 80 --search_patience 15
"""
...
parser.add_argument('--checkpoint', type=str, default='./checkpoint/cp_contrastive')
...
def build_motion_modes(hp_config, args):
    ...
    if not os.path.exists(motion_modes_file):
        ...
        motion_modes = get_motion_modes(
            train_dataset,              # 첫 번째 인자는 dataset (키워드 X)
...
def train_one_epoch(epoch, model, reg_criterion, cls_criterion,
    ...
    for (ped, neis, mask, cp) in dataloader:  # ✅ cp 추가
        ped, neis, mask, cp = ped.cuda(), neis.cuda(), mask.cuda(), cp.cuda()
...
def test(model, dataloader, motion_modes):
    model.eval()
    ade, fde, num_traj = 0, 0, 0
    rmse_x_sum, rmse_y_sum = 0, 0
    inference_times = []
    for batch in dataloader:
        # ✅ 배치가 4개 항목일 수도 있음 (cp 포함)
        if len(batch) == 4:
            ped, neis, mask, _ = batch  # cp 무시
        else:
            ped, neis, mask = batch
        ...
        # 원래 코드처럼 top-k=20 고정 (필요하면 hp_config.PRED_SAMPLES로 바꿀 수 있음)
