...
class TargetBuilder(BaseTransform):
    ...
    def __call__(self, data: HeteroData) -> HeteroData:
        ...
        data['agent']['target_global'] = origin.new_zeros(data['agent']['num_nodes'], self.num_future_steps, 4) 
        data['agent']['target_global'][..., :2] = torch.bmm(data['agent']['target'][..., :2], inv_rot_mat) + origin[:, :2].unsqueeze(1) 
        # jsa add   
