...
class RMSEX_global(Metric):

    def __init__(self,
                 max_guesses: int = 6,
                 **kwargs) -> None:
        super(RMSEX_global, self).__init__(**kwargs)
    ...
    def update(self,
        ...
        self.sum += ((((pred_topk[torch.arange(pred.size(0)), inds_best, :, 0] - target[..., 0])**2)* valid_mask).sum(dim=-1) / valid_mask.sum(dim=-1)).sum()
        pred_tmp = pred_topk[torch.arange(pred.size(0)), inds_best, :, 0]
        gt_tmp = target[..., 0]
