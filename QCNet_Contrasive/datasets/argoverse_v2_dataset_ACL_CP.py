...
class ArgoverseV2Dataset_ACL_CP(Dataset):
    ...
    def __init__(self,
        ...
        super(ArgoverseV2Dataset_ACL_CP, self).__init__(root=root, transform=transform, pre_transform=None, pre_filter=None)
    ...
    def get_agent_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for track_id, track_df in df.groupby('track_id'):
            ...
            difficulty = torch.zeros(num_agents, dtype=torch.float) 
            # cp_max를 난이도로 추가 
            if 'cp_max' in track_df.columns: 
                val = track_df['cp_max'].iloc[0] 
                difficulty[agent_idx] = torch.tensor(val, dtype=torch.float) 
            else: 
                # cp_max 없는 경우 0.0으로 초기화 
                difficulty[agent_idx] = 0.0
    ...
    def get(self, idx: int) -> HeteroData:
        ...
        # ✅ 난이도 값이 존재할 경우 agent 노드에 연결
        if 'difficulty' in data_dict.get('agent', {}):
            diff = data_dict['agent']['difficulty']
            if not torch.is_tensor(diff):
                diff = torch.tensor(diff, dtype=torch.float32)
            data['agent']['difficulty'] = diff
        else:
            print(f"[경고] difficulty 값이 없는 샘플: {self.processed_paths[idx]}")
